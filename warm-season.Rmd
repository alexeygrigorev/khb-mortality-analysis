...
default.par = par()
setwd("C:/Users/Alexey Grigorev/YandexDisk/Mama/mort-temp/")
# setwd("~/docs/mam/r-regression-analysis")
...
# mort0 = merge(mort, temp, by='date', all.x=T)
# View(mort0[!complete.cases(mort0), ])
# need to get data for last septermber!
...
add.fit = function(fit, x) {
  ...
  polygon(x=c(x, rev(x)),
          ...
          col=adjustcolor('grey', 1/8), border=NA)
  
  lines(x, pred[,2], col='grey', lwd=1)
  lines(x, pred[,3], col='grey', lwd=1)
...
```{r fig.width=7, fig.height=4, warning=FALSE}
Cairo(width=30, height=16, units='cm', file='mtt.png', 
      bg='white',  pointsize=12, dpi=72)
